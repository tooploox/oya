...
// Package internal implements parallel execution helpers.
...
type Work struct {
	f       func(Job) // function to run for each item
	running int       // total number of runners

	mu      sync.Mutex
	added   map[interface{}]bool // items added to set
	todo    []Job                // items yet to be run
...
func (w *Work) Add(item Job) {
	w.mu.Lock()
	w.init()
	if !w.added[item.ID()] {
		w.added[item.ID()] = true
...
func (w *Work) Do(n int, f func(item Job)) {
	if n < 1 {
