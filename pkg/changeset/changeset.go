...
func Calculate(candidates []*oyafile.Oyafile) ([]*oyafile.Oyafile, error) {
	...
	// Assumes oyafiles is generated by traversing directories top-first.
	// BUG(bilus): This precondition can be tested here.
	rootOyafile := candidates[0]
	_, ok := rootOyafile.Tasks["Changeset"]
	if !ok {
		rootOyafile.Tasks["Changeset"] = defaultRootChangesetTask(candidates)
	}
