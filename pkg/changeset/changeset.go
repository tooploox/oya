...
func (vo *VisitedOyafiles) Visit(o *oyafile.Oyafile) {
	if vo.IsVisited(o) {
...
func Calculate(o *oyafile.Oyafile) ([]*oyafile.Oyafile, error) {
	o.Hooks["Changeset"] = rootChangesetHook(o) // Set to default if not present.
	return calculateChangeset(o.Dir, o, new(VisitedOyafiles))
}
...
func calculateChangeset(rootDir string, currOyafile *oyafile.Oyafile, visited *VisitedOyafiles) ([]*oyafile.Oyafile, error) {
	var changeset []*oyafile.Oyafile

	visited.Visit(currOyafile)
	oyafiles, err := doCalculateChangeset(rootDir, currOyafile)
	if err != nil {
		return nil, err
	}
	oyafiles = unique(oyafiles)

	changeset = append(changeset, oyafiles...)

	for _, o := range oyafiles {
		if visited.IsVisited(o) {
			continue
		}
		localChangeset, err := calculateChangeset(rootDir, o, visited)
		if err != nil {
			return nil, err
		}
		changeset = append(changeset, localChangeset...)
	}
